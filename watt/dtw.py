...
def derivative_dtw(q):
    d = [0.0]*len(q)
    d[0] = q[0]
    d[-1] = q[-1]
    for i in range(1,len(q)-1):
        d[i]=((q[i]-q[i-1])+ ((q[i+1]-q[i-1])/2.0))/2.0
    return d
...
def find_peak(q):
    ...
    d[-1] = q[-1]
    for i in range(1,len(q)-1):
        d[i]= (q[i-1]-q[i])*(q[i+1]-q[i])
    return d



def ddtw(x, y, dist=lambda x, y: norm(x - y, ord=1)):
    ...
    """
    x = array(derivative_dtw(x))
    if len(x.shape) == 1:
        x = x.reshape(-1, 1)
    y = array(derivative_dtw(y))
    if len(y.shape) == 1:
        y = y.reshape(-1, 1)
